...
/**
 * All credits for this class go to Jofkos - https://github.com/Jofkos
 * The gist this class was taken from - https://gist.github.com/Jofkos/79af290e94acdc7d7d5b
 */
public class GameProfileBuilder {

    private static final String SERVICE_URL = "https://sessionserver.mojang" +
            ".com/session/minecraft/profile/%s?unsigned=false";
    private static final String UUID_URL = "https://https://api.minetools.eu/uuid/%s";
    private static final String JSON_SKIN = "{\"timestamp\":%d,\"profileId\":\"%s\",\"profileName\":\"%s\"," +
            "\"isPublic\":true,\"textures\":{\"SKIN\":{\"url\":\"%s\"}}}";
    private static final String JSON_CAPE = "{\"timestamp\":%d,\"profileId\":\"%s\",\"profileName\":\"%s\"," +
            "\"isPublic\":true,\"textures\":{\"SKIN\":{\"url\":\"%s\"},\"CAPE\":{\"url\":\"%s\"}}}";

    private static Gson gson = new GsonBuilder().disableHtmlEscaping().registerTypeAdapter(UUID.class,
    ...
    public static GameProfile fetch(String name) throws IOException {
        ...
        if (connection.getResponseCode() == HttpURLConnection.HTTP_OK) {
            ...
            new BufferedReader(new InputStreamReader(connection.getInputStream())).lines().forEach(json::append);
            JsonObject object = gson.fromJson(json.toString(), JsonObject.class);
            return fetch(UUID.fromString(object.get("id").getAsString()));
        } else {
            throw new IOException("Failed to parse uuid of " + name);
        }
    }
    ...
    public static GameProfile fetch(UUID uuid, boolean forceNew) throws IOException {
        if (!forceNew && cache.containsKey(uuid) && cache.get(uuid).isValid()) {
            ...
            if (connection.getResponseCode() == HttpURLConnection.HTTP_OK) {
                ...
                JsonObject error =
                        (JsonObject) new JsonParser().parse(new BufferedReader(new InputStreamReader(connection.getErrorStream())).readLine());
    ...
    public static GameProfile getProfile(UUID uuid, String name, String skinUrl, String capeUrl) {
        ...
        profile.getProperties().put("textures", new Property("textures", Base64Coder.encodeString(String.format(cape
                ? JSON_CAPE : JSON_SKIN, args.toArray(new Object[args.size()])))));
        return profile;
    }
